...
-(instancetype)initWithCoder:(NSCoder *)coder{
    NVTextCollectionView *view = [super initWithCoder:coder];
    view.source = [[NVTextSource alloc]init];
...
-(NSCollectionViewItem *)collectionView:(NSCollectionView *)collectionView itemForRepresentedObjectAtIndexPath:(NSIndexPath *)indexPath{
    NVTextSpec spec = [self.source getSpecAt:indexPath];
    NSString * className= self.semanticMode ? @"NVTextCollectionItem" : @"NVTextCollectionItem";
    NSCollectionViewItem *cell = [collectionView makeItemWithIdentifier:className forIndexPath:indexPath];
    
//    if (self.semanticMode) {
//        NVTextSemanticItemView *item = cell.view;
//        item.spec = spec;
...
//        }];
//    }else {
        NVTextCollectionItemView *item = cell.view;
        item.spec = spec;
        item.indexPath = indexPath;
        [item onMouseDown:^void(NSEvent* event, NSBox* box) {
            [self.toggleDelegate clearActive];
...
- (NSView *)collectionView:(NSCollectionView *)collectionView viewForSupplementaryElementOfKind:(NSCollectionViewSupplementaryElementKind)kind atIndexPath:(NSIndexPath *)indexPath {
    if (kind == NSCollectionElementKindSectionHeader) {
        NVTextSectionHeader *view =
        [self makeSupplementaryViewOfKind:kind withIdentifier: @"NVTextSectionHeader" forIndexPath:indexPath];
