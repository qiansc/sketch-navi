        ...
        if (box == nil) {
            ...
            if ([selectedSpecs[spec.pos] isEqual:spec.code]) {
                ...
                if ([self relationOf:layer and:target] == 0) {
                    switch (spec.pos) {
                        case 0: selectedSpecs[2] = @""; [NVUserInfo fromLayer:layer].marginBottomCode=nil;break;
                        case 2: selectedSpecs[0] = @"";[NVUserInfo fromLayer:layer].marginTopCode=nil;break;
                        case 1: selectedSpecs[3] = @"";[NVUserInfo fromLayer:layer].marginLeftCode=nil;break;
                        case 3: selectedSpecs[1] = @"";[NVUserInfo fromLayer:layer].marginRightCode=nil;break;
                        default:
                            break;
                    }
                }
...
//        for(MSLayer *layer in self.selections) {
        if ([NVLayer isShape:layer]){
            ...
            if (target) {
                [NVUserInfo fromLayer:layer].marginType = [NSString stringWithFormat:@"%i",
...
-(void)applyOuterBottom {
    ...
    NVMarginSpec spec = [self.collectionView.dataSource getSpecWithCode:selectedSpecs[0]];
    if (spec.code == nil) return;
    NVMarginSpec topSpec = [self.collectionView.dataSource getSpecWithCode:selectedSpecs[2]];
    if (topSpec.code) {
        return;
    }
